...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    // If the endpoint requires a role but there is no authenticated user, deny.
    // (JwtAuthGuard already runs first via APP_GUARD ordering, so this should
    //  only be reached if somehow an unauthenticated request bypassed JWT.)
    if (!user) {
    ...
    // Resolve the minimum level required from the list.
